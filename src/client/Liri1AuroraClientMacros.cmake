...
function(liri_generate_wayland_protocol_client_sources target)
    cmake_parse_arguments(_arg "" "" "FILES" ${ARGN})
    if(DEFINED _arg_UNPARSED_ARGUMENTS)
        message(FATAL_ERROR "Unknown arguments passed to liri_generate_wayland_protocol_client_sources: (${_arg_UNPARSED_ARGUMENTS}).")
    endif()
    ...
    foreach(protocol_file IN LISTS _arg_FILES)
        ...
        add_custom_command(
            ...
            COMMAND Liri::waylandscanner client-header "${protocol_file}" "" > "${cpp_header_path}"
            DEPENDS "${protocol_file}" "${c_header_path}"
        )
        add_custom_command(
            OUTPUT "${cpp_code_path}"
            COMMAND Liri::waylandscanner client-code "${protocol_file}" "" > "${cpp_code_path}"
