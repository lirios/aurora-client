...
Q_LOGGING_CATEGORY(gLcWlrForeignToplevelManagementV1, "aurora.client.wlrforeigntoplevelmanagementv1")
...
void WlrForeignToplevelHandleV1Private::zwlr_foreign_toplevel_handle_v1_output_enter(wl_output *output)
{
    ...
    if (!screen) {
        qCWarning(gLcWlrForeignToplevelManagementV1, "Cannot find a screen for wl_output");
        return;
    }
...
void WlrForeignToplevelHandleV1::setFullscreen(QScreen *screen)
{
    ...
    if (!output) {
        qCWarning(gLcWlrForeignToplevelManagementV1, "Cannot find wl_output for screen %s",
