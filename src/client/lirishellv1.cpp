...
Q_LOGGING_CATEGORY(gLcLiriShellV1, "aurora.client.lirishellv1", QtInfoMsg)
...
void LiriShortcutV1::setShell(LiriShellV1 *shell)
{
    ...
    if (d->isInitialized()) {
        qCWarning(gLcLiriShellV1, "Cannot set LiriShortcutV1::shell after the shortcut is bound");
        return;
    }
...
void LiriShortcutV1::setSequence(const QString &sequence)
{
    ...
    if (d->isInitialized()) {
        qCWarning(gLcLiriShellV1, "Cannot set LiriShortcutV1::sequence after the shortcut is bound");
        return;
    }
...
void LiriShortcutV1::bindShortcut()
{
    ...
    if (d->isInitialized()) {
        qCWarning(gLcLiriShellV1, "Cannot bind LiriShortcutV1 twice");
        return;
    }

    if (!d->shell) {
        qCWarning(gLcLiriShellV1, "Cannot bind shortcut since LiriShortcutV1::shell is not set");
        return;
    }

    if (d->sequence.isEmpty()) {
        qCWarning(gLcLiriShellV1, "Cannot bind shortcut since LiriShortcutV1::sequence is not set");
        return;
    }
